...
.PHONY: mlb-cleanroom-outcome-closeout-lineage-status mlb-cleanroom-bol-tb15-pregame-freeze mlb-cleanroom-bol-tb15-lifecycle-status mlb-cleanroom-bol-tb15-fixed-cohort mlb-cleanroom-bol-tb15-fixed-cohort-closeout mlb-cleanroom-bol-tb15-fixed-cohort-status mlb-cleanroom-bol-tb15-fixed-cohort-block-status mlb-cleanroom-bol-tb15-schedule-cohort mlb-cleanroom-bol-tb15-schedule-cohort-closeout mlb-cleanroom-bol-tb15-schedule-cohort-status mlb-cleanroom-bol-tb15-schedule-cohort-block-status mlb-cleanroom-bol-tb15-routine-cohort mlb-cleanroom-bol-tb15-routine-cohort-closeout mlb-cleanroom-bol-tb15-routine-cohort-status mlb-cleanroom-bol-tb15-routine-verification-status mlb-cleanroom-routine-outcome-audit mlb-player-stats-game-completeness mlb-player-stats-date-completeness mlb-player-stats-repair-game mlb-player-stats-recover-date
...
mlb-cleanroom-bol-tb15-pregame-freeze:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.closeout_cleanroom_bol_tb15 \
		--date "$(MLB_DATE)" --freeze-only

mlb-cleanroom-bol-tb15-lifecycle-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.closeout_cleanroom_bol_tb15 \
		--date "$(MLB_DATE)" --lifecycle-status

mlb-cleanroom-bol-tb15-fixed-cohort:
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.fixed_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode capture

mlb-cleanroom-bol-tb15-fixed-cohort-closeout:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.fixed_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode closeout

mlb-cleanroom-bol-tb15-fixed-cohort-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.fixed_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode status

mlb-cleanroom-bol-tb15-fixed-cohort-block-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.fixed_cohort_lifecycle --mode block-status

mlb-cleanroom-bol-tb15-schedule-cohort:
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.schedule_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode capture

mlb-cleanroom-bol-tb15-schedule-cohort-closeout:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.schedule_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode closeout

mlb-cleanroom-bol-tb15-schedule-cohort-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.schedule_cohort_lifecycle \
		--date "$(MLB_DATE)" --mode status

mlb-cleanroom-bol-tb15-schedule-cohort-block-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.schedule_cohort_lifecycle --mode block-status

mlb-cleanroom-bol-tb15-routine-cohort:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.routine_market_lifecycle --mode cohort --date $(MLB_DATE) $(if $(MLB_ROUTINE_RUN_TAG),--run-tag $(MLB_ROUTINE_RUN_TAG),)

mlb-cleanroom-bol-tb15-routine-cohort-closeout:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.routine_market_lifecycle --mode closeout --date $(MLB_DATE)

mlb-cleanroom-bol-tb15-routine-cohort-status:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.routine_market_lifecycle --mode status --date $(MLB_DATE)

mlb-cleanroom-bol-tb15-routine-verification-status:
	@$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.routine_market_lifecycle --mode verification-status

mlb-cleanroom-routine-outcome-audit:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.cleanroom_v1.routine_outcome_reconciliation --date "$(MLB_DATE)"

mlb-player-stats-game-completeness:
	@test -n "$(MLB_GAME_PK)" || (echo "MLB_GAME_PK is required" >&2; exit 2)
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.player_stats_game_completeness --mode game --game-pk "$(MLB_GAME_PK)" $(if $(MLB_PLAYER_STATS_OUTPUT_DIR),--output-dir "$(MLB_PLAYER_STATS_OUTPUT_DIR)",)

mlb-player-stats-date-completeness:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.player_stats_game_completeness --mode date --date "$(MLB_DATE)" $(if $(MLB_PLAYER_STATS_OUTPUT_DIR),--output-dir "$(MLB_PLAYER_STATS_OUTPUT_DIR)",)

mlb-player-stats-repair-game:
	@test -n "$(MLB_GAME_PK)" || (echo "MLB_GAME_PK is required" >&2; exit 2)
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.player_stats_game_completeness --mode repair --game-pk "$(MLB_GAME_PK)" $(if $(MLB_PLAYER_STATS_OUTPUT_DIR),--output-dir "$(MLB_PLAYER_STATS_OUTPUT_DIR)",)

mlb-player-stats-recover-date:
	@test -n "$(MLB_DATE)" || (echo "MLB_DATE is required" >&2; exit 2)
	@set -a; . backend/.env; set +a; \
	$(VENV_PY) -u -m backend.mlb.scripts.player_stats_game_completeness --mode recover-date --date "$(MLB_DATE)" $(if $(MLB_PLAYER_STATS_OUTPUT_DIR),--output-dir "$(MLB_PLAYER_STATS_OUTPUT_DIR)",)
