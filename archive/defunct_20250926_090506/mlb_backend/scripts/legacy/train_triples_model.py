...
MODEL_OUTPUT_PATH = "models/Triples_model.pkl"

# --- Supabase client ---
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

# --- Fetch triples data ---
def fetch_triples_data():
    response = supabase.table("model_training_props").select("*").eq("prop_type", "Triples").in_("outcome", ["win", "loss"]).execute()
...
# --- Main ---
def main():
    print("📦 Fetching data for 'Triples'...")
    df = fetch_triples_data()
    if df.empty:
        print("⚠️ No data found for 'Triples'.")
        return
