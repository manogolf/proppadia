...
if [[ "$lifecycle_rc" -eq 0 ]]; then
  ...
  echo "[${finished_at}] WARN MLB totals prospective shadow lifecycle failed slate_date=${slate_date} exit_status=${lifecycle_rc}; normal MLB refresh continues and a later run may retry" >&2
fi
exit "$lifecycle_rc"
