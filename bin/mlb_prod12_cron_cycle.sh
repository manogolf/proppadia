...
if [[ "${MLB_PROD12_DAILY_PROP_TYPES}" != "${MLB_PROD12_PROP_TYPES}" ]]; then
  echo "[prod12-cron] WARN: ignoring narrowed MLB_PROD12_DAILY_PROP_TYPES for full daily eval; using full MLB_PROD12_PROP_TYPES"
fi
...
MLB_BOOK_UPLOAD_MIN_SIDE_PROB="${MLB_BOOK_UPLOAD_MIN_SIDE_PROB:-0}"
...
MLB_POLICY_PLAN_ENABLED="${MLB_POLICY_PLAN_ENABLED:-0}"
...
run_daily() {
  ...
  if [[ "${MLB_DAILY_WIDE_PREDICTIONS_ENABLED}" == "1" ]]; then
    ...
    MLB_PREDICT_REQUIRE_TWO_SIDED="${MLB_PREDICT_REQUIRE_TWO_SIDED}" \
    MLB_PREDICT_TWO_SIDED_BOOKMAKER="${MLB_PREDICT_TWO_SIDED_BOOKMAKER}" \
  ...
  if [[ "${MLB_DAILY_SLATE_ARTIFACTS_ENABLED}" == "1" ]]; then
    ...
    MLB_BOOK_UPLOAD_MIN_SIDE_PROB="${MLB_BOOK_UPLOAD_MIN_SIDE_PROB}" \
