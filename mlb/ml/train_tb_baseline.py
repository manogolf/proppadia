...
# Config
# =========================
LINE = 1.5  # e.g., 0.5 / 1.5 / 2.5
ID_COLS = ["game_id", "player_id", "game_date"]  # not used as features
TARGET = "y_tb"

CSV_PATH = Path(__file__).with_name("train_batter_total_bases.csv")
OUT_PATH = Path(__file__).with_name("pred_tb_test.csv")
TEST_DAYS = 21
VAL_DAYS = 14
...
try:
    # Works whether OHE was fitted or skipped
    all_names = list(pre_fitted.get_feature_names_out())
except Exception:
    # Fallback if older sklearn / unexpected behavior
    cat_names = []
    if len(cat_cols) > 0:
...
if imps is not None and len(all_names) == len(imps):
    ...
    # In case dimensions don't match (e.g., different sklearn behavior), just skip
...
# Prevalence / constant Brier for LINE=1.5
yt15 = (np.asarray(y_test) > 1.5).astype(int)
prev15 = float(np.mean(yt15))
brier_const15 = prev15 * (1.0 - prev15)
print(f"Test prevalence TB≥1.5 = {prev15:.3f}  |  Constant baseline Brier = {brier_const15:.4f}")
...
# collect logistic calibrators here
calibrators = {}  # e.g. {"0_5": {"intercept": ..., "coef": ..., "line": 0.5}, ...}
...
def calibrate_over_line(line, name=None):
    ...
    # ---- save calibrator for later inference ----
    # Isotonic can be reconstructed from its breakpoints
    calibrators[name] = {
        "line": float(line),
        "x": iso.X_thresholds_.tolist(),  # sorted breakpoints of raw preds
        "y": iso.y_thresholds_.tolist(),  # calibrated probs at those points
    }
...
# Calibrate common TB lines
for L in (0.5, 1.5, 2.5):
    calibrate_over_line(L)

with open(models_dir / "tb_calibrators_v1.json", "w") as f:
    json.dump(calibrators, f, indent=2)
print(f"Saved calibrators to {models_dir/'tb_calibrators_v1.json'}")


# Write predictions CSV
out.to_csv(OUT_PATH, index=False)
print(f"\nWrote {OUT_PATH} with calibrated probabilities for 0.5 / 1.5 / 2.5")

# =========================
# Save model EXACTLY as calibrated & evaluated (train-only fit)
# =========================
dump(pipe, models_dir / "tb_poisson_v1.joblib")
with open(models_dir / "tb_features_v1.json", "w") as f:
    json.dump(list(X.columns), f)
print(f"\nSaved model to {models_dir/'tb_poisson_v1.joblib'}")
print(f"Saved feature list to {models_dir/'tb_features_v1.json'}")