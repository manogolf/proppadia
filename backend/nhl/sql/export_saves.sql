...
SELECT to_regclass('nhl.roster_daily')          IS NOT NULL AS has_roster \gset
...
\if :has_view
  ...
  \if :has_rows
    -- Schema preflight (same as before)
    DO $$
    ...
    -- Copy rows with full_name selection depending on availability
    \if :has_roster
      COPY (
        SELECT
          COALESCE(r.full_name, p.full_name, p.name) AS "full_name",
  ...
  \else
    -- Header only (no rows). Keep full_name column.
    COPY (
      SELECT
        NULL::text    AS full_name,
