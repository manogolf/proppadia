-- backend/nhl/sql/export_saves_from_denali.sql
...
--     -f backend/nhl/sql/export_saves_from_denali.sql > backend/nhl/exports/train_goalie_saves_v2.csv
...
WITH base AS (
  ...
  -- restrict to *goalies only* by requiring that the player_id exists in goalie_game_logs_raw
  JOIN (
    SELECT DISTINCT player_id
    FROM nhl.goalie_game_logs_raw
  ) gg
    ON gg.player_id = rs.player_id
