...
pairings_features_per_game AS (
  ...
  -- TOI from shiftcharts_shifts, restricted to prior_games only
  LEFT JOIN (
    SELECT
      s.game_id::bigint,
      s.player_id::bigint,
      SUM(s.dur_sec)::int AS toi_sec
    FROM nhl.shiftcharts_shifts s
    JOIN prior_games pg2
      ON pg2.game_id = s.game_id
    GROUP BY 1,2
  ) t
    ON t.game_id = f.game_id
   AND t.player_id = f.player_id
...
final_values AS (
  SELECT
    ...
    -- season TOI (Option B passthrough; used by probe + downstream)
    s.szn_toi_per_game_5on5,
    s.szn_toi_per_game_pp,
    s.szn_toi_per_game_pk,
    s.season_5on5_icetime_per_game,
    s.season_5on4_icetime_per_game,
    s.season_4on5_icetime_per_game,
    ...
    -- keep strict meaning (NULL unless you later choose to backfill from d20)
    ...
    (COALESCE(r10.shiftcharts_games, 0) > 0) AS d10_pairings_available,
    (COALESCE(r20.shiftcharts_games, 0) > 0) AS d20_pairings_available,
...
-- === APPLY: write final_values into the base table for slate_date ===
applied AS (
  ...
  SET
    ...
    pairings_source                    = 'fill_sog_pairings_for_slate.sql',
    pairings_updated_at                = NOW(),
    mate_stability_source              = 'fill_sog_pairings_for_slate.sql',
    mate_stability_updated_at          = NOW()

  FROM final_values fv
  JOIN params p ON TRUE
  WHERE t.game_date::date = p.slate_date
    AND t.player_id = fv.player_id
    AND t.game_id   = fv.game_id
  RETURNING 1
)

-- optional: keep a small confirmation summary
SELECT
  (SELECT COUNT(*) FROM final_values) AS n_final_values,
  (SELECT COUNT(*) FROM applied)      AS n_rows_updated;