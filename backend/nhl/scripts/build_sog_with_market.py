...
def main():
    ...
    # --- parse odds JSON (player_shots_on_goal) and build median Over price by (name_short,line) ---
            ...
            if isinstance(x, dict):
                if x.get("key") == "player_shots_on_goal":
                    for o in x.get("outcomes", []) or []:
                        if o.get("name") != "Over":
                            continue
                        base_name = o.get("description") or o.get("player") or ""
                        nm_norm = norm_name(base_name)
