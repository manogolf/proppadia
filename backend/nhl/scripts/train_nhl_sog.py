#!/usr/bin/env python3
"""
NHL v1.1 — Train Shots-on-Goal (SOG) with Poisson vs Negative Binomial,
temporal CV, **per-line isotonic calibration**, and clean artifacts.

This is a drop-in replacement for your current train_nhl_sog.py.
New flag: --calibration-days (defaults to 7). The most recent
'calibration-days' before holdout are used to fit isotonic calibrators
for each --eval-lines value. Holdout remains untouched for unbiased eval.

Usage (example):
  python train_nhl_sog.py \
      --csv data/processed/nhl_sog_training.csv \
      --date-col game_date \
      --label-col shots_on_goal \
      --feature-json features/feature_metadata_nhl.json \
      --feature-key shots_on_goal \
      --out-dir models/latest/shots_on_goal \
      --eval-lines 2.5,3.5 \
      --n-folds 5 \
      --holdout-days 7 \
      --calibration-days 7
"""
import argparse, os, json, hashlib, warnings
...
def make_temporal_folds(df: pd.DataFrame, date_col: str, n_folds: int, gap_days:int=2) -> List[Tuple[np.ndarray, np.ndarray]]:
    ...
    if len(unique_days) < n_folds + 2:
        return []
    fold_edges = np.linspace(0, len(unique_days), n_folds + 1).astype(int)
    folds = []
...
def interp_calibrate(p_raw: np.ndarray, grid_x: np.ndarray, grid_y: np.ndarray) -> np.ndarray:
...
def run_cv_and_select(
    ...
    for a in alpha_grid_poisson:
        ...
        for tr_idx, va_idx in folds:
            ...
            for line in eval_lines:
                y_over = (yv > line).astype(int)
                p_over = np.clip(prob_over_poisson(mu_val, line), 1e-6, 1 - 1e-6)
    ...
    for disp_alpha in alpha_grid_nb:
        ...
        for tr_idx, va_idx in folds:
            ...
            for line in eval_lines:
                y_over = (yv > line).astype(int)
                p_over = np.clip(prob_over_nb(mu_val, disp_alpha, line), 1e-6, 1 - 1e-6)
                b_line.append(brier_score(y_over, p_over))
                try: ll_line.append(log_loss(y_over, p_over, labels=[0,1]))
                except ValueError: ll_line.append(np.nan)
                if y_over.min() != y_over.max():
...
# --- REPLACE your existing fit_isotonic_per_line with this guarded version ---
# In scripts/train_nhl_sog.py, find this function and replace just the def line:
def fit_isotonic_per_line(
    model_obj, family: Dict, params: Dict,
    df_cal: pd.DataFrame, features: List[str], label_col: str, eval_lines: List[float],
    min_per_class: int = 10,  # was 30; allow calibration with fewer negatives
    improve_tol: float = 0.005
) -> Dict[str, Dict]:
    """
    Fit isotonic per line on a calibration window, but only keep a calibrator
    if it *improves* log loss or ECE on that window and has enough class balance.
    Returns a dict of {line_str: {grid_x, grid_y, n, diagnostics...}}.
    """
    if df_cal is None or df_cal.empty:
        return {}

    Xc = prepare_features(df_cal, features)
    yc_counts = df_cal[label_col].astype(int).values
    out = {}

    for L in eval_lines:
        y_over = (yc_counts > L).astype(int)
        n_pos = int(y_over.sum())
        n_neg = int(len(y_over) - n_pos)
        ...
        # Keep only if it clearly helps on *either* LL or ECE
...
def eval_lines_metrics(y_true_counts: np.ndarray, p_map: Dict[float, np.ndarray]) -> Dict[str, Dict]:
    """Compute metrics for each line given map of line->probabilities."""
    metrics = {}
    for L, p_over in p_map.items():
        y_over = (y_true_counts > L).astype(int)
        p = np.clip(p_over, 1e-6, 1-1e-6)
        m = {
            "brier": float(brier_score(y_over, p)),
        }
        try:
            m["log_loss"] = float(log_loss(y_over, p, labels=[0,1]))
        except ValueError:
            m["log_loss"] = float("nan")
        if y_over.min() != y_over.max():
            try:
                m["auc"] = float(roc_auc_score(y_over, p))
            except Exception:
                m["auc"] = None
        else:
            m["auc"] = None
...
def save_artifacts(
    ...
    model_artifact = {
        ...
        "calibration": calibration_payload or {},  # per-line grids if present
    }
...
def main():
    ...
    ap.add_argument("--label-col", default="shots_on_goal")
    ap.add_argument("--feature-json", required=True)
    ap.add_argument("--feature-key", default="shots_on_goal")
    ap.add_argument("--out-dir", required=True)
    ap.add_argument("--eval-lines", default="2.5,3.5")
    ap.add_argument("--n-folds", type=int, default=5)
    ap.add_argument("--holdout-days", type=int, default=14)
    ap.add_argument("--calibration-days", type=int, default=7)  # NEW
    ...
    # Temporal splits: [TRAIN_CORE] ... [CALIBRATION] ... [HOLDOUT]
    ...
    # CV on TRAIN_CORE and select winner
    ...
    # Fit per-line isotonic on CALIBRATION (if available)
    calibration_payload = {}
    if not df_calib.empty:
        ...
        if calibration_payload:
            ...
            print("ℹ️ Skipped calibration (no df_calib rows).")

    # Holdout evaluation (raw + calibrated if available)
    ...
    if not df_holdout.empty:
        ...
        raw_map = {}
        for L in eval_lines:
            raw_map[L] = predict_family(model_obj, fam, Xh, best.params.get("disp_alpha"), L)
        ...
        if calibration_payload:
            ...
            for L in eval_lines:
                ...
                if cal:
                    ...
                    cal_map[L] = interp_calibrate(raw_map[L], gx, gy)
            if cal_map:
                metrics_holdout_cal = eval_lines_metrics(yh, cal_map)

    # Save artifacts + indices
    save_artifacts(
    ...
    # Save feature hash file (for scorers to sanity-check)
    with open(os.path.join(args.out_dir, "FEATURE_HASH.txt"), "w") as f:
