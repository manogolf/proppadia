#!/usr/bin/env python3
"""Evaluate exact on-ice opposing-defender deployment with player-normalized position attribution."""
...
SELECT
  ...
  COALESCE(s.dur_sec, GREATEST(0, s.end_sec - s.start_sec))::int AS dur_sec
...
SELECT
  ...
  shooting_team_id::bigint AS shooting_team_id,
  defending_team_id::bigint AS defending_team_id,
  COALESCE(NULLIF(BTRIM(shooter_position_bucket), ''), 'F') AS shooter_position_bucket,
  event_abs_sec::int AS event_abs_sec
FROM nhl.shot_on_goal_events
WHERE season = %s
  AND (%s::date IS NULL OR game_date >= %s::date)
  AND (%s::date IS NULL OR game_date <= %s::date)
  AND event_abs_sec IS NOT NULL
ORDER BY game_date, game_id, event_abs_sec
"""
...
def _is_on_ice(intervals: List[Tuple[int, int]], sec: int) -> bool:
...
def _build_defender_prior_rate_map(shifts_df: pd.DataFrame, shots_df: pd.DataFrame) -> Dict[Tuple[int, int, str], float]:
    defender_game_toi: Dict[Tuple[int, int, int], float] = {}
    fractional_counts: Dict[Tuple[int, int, int, str], float] = defaultdict(float)
    ...
    for game_id, game_shifts in shifts_df.groupby("game_id", sort=False):
        ...
        defenders_by_team: Dict[int, Dict[int, List[Tuple[int, int]]]] = defaultdict(dict)
        offense_by_team_bucket: Dict[Tuple[int, str], Dict[int, List[Tuple[int, int]]]] = defaultdict(dict)
        game_date = None
        ...
        for row in game_shifts.itertuples(index=False):
            ...
            if pos == "G":
                continue
            offense_by_team_bucket.setdefault((team_id, pos), {}).setdefault(player_id, []).append(interval)
            if pos == "D":
                defenders_by_team.setdefault(team_id, {}).setdefault(player_id, []).append(interval)
                defender_game_toi[(game_id, team_id, player_id)] = defender_game_toi.get((game_id, team_id, player_id), 0.0) + float(row.dur_sec or 0.0)

        if game_date is None:
            continue

        for row in game_shots.itertuples(index=False):
            sec = int(row.event_abs_sec)
            shooting_team = int(row.shooting_team_id)
            defending_team = int(row.defending_team_id)
            shooter_bucket = _norm_pos(row.shooter_position_bucket)
            if shooter_bucket == "G":
                continue
            offense_players = offense_by_team_bucket.get((shooting_team, shooter_bucket), {})
            defenders = defenders_by_team.get(defending_team, {})
            if not offense_players or not defenders:
                continue
            same_bucket_on_ice = sum(1 for intervals in offense_players.values() if _is_on_ice(intervals, sec))
            if same_bucket_on_ice <= 0:
                continue
            defenders_on_ice = [defender_id for defender_id, intervals in defenders.items() if _is_on_ice(intervals, sec)]
            if not defenders_on_ice:
                continue
            frac = 1.0 / float(same_bucket_on_ice)
            for defender_id in defenders_on_ice:
                fractional_counts[(game_id, defending_team, defender_id, shooter_bucket)] += frac

    rows: List[Dict[str, Any]] = []
    for (game_id, defending_team, defender_id), toi_sec in defender_game_toi.items():
        if toi_sec <= 0:
            continue
        game_date = None
        # game_date recover from shifts_df subset
        # choose first matching row by boolean mask once per defender-game
        match = shifts_df[
            (shifts_df["game_id"] == game_id)
            & (shifts_df["team_id"] == defending_team)
            & (shifts_df["player_id"] == defender_id)
        ]
        if not match.empty:
            game_date = str(match.iloc[0]["game_date"])
        for shooter_bucket in POSITION_BUCKETS:
            rows.append(
                {
                    "game_date": game_date,
                    "game_id": int(game_id),
                    "defending_team_id": int(defending_team),
                    "defender_id": int(defender_id),
                    "shooter_position_bucket": shooter_bucket,
                    "toi_minutes": float(toi_sec) / 60.0,
                    "sog_allowed_playernorm": float(
                        fractional_counts.get((game_id, defending_team, defender_id, shooter_bucket), 0.0)
                    ),
                }
            )
    ...
    if hist.empty:
        return {}
    hist = hist.sort_values(["defender_id", "shooter_position_bucket", "game_date", "game_id"])

    out: Dict[Tuple[int, int, str], float] = {}
    for (defender_id, shooter_bucket), grp in hist.groupby(["defender_id", "shooter_position_bucket"], sort=False):
        ...
        for row in grp.itertuples(index=False):
            ...
            if last10:
                ...
                out[key] = math.nan
            last10.append((float(row.sog_allowed_playernorm or 0.0), float(row.toi_minutes or 0.0)))
    return out


def _build_test_overlap_map(shifts_df: pd.DataFrame, test_rows: pd.DataFrame) -> Dict[Tuple[int, int], Dict[int, int]]:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Evaluate exact on-ice opposing-defender deployment with player-normalized position attribution."
    )
    ...
    test_shifts = shifts_df[
        shifts_df["game_date"].astype(str).isin(test_dates)
    ].copy()
    ...
    scored["matchup_defender_playernorm_allowed_d10_per60"] = matchup_rates
    ...
    scored["lambda_defense_matchup"] = (
        pd.to_numeric(scored["matchup_defender_playernorm_allowed_d10_per60"], errors="coerce").clip(lower=0.0)
    ...
    scored.loc[both, "lambda_combined_matchup"] = (
        (scored.loc[both, "rate_offense"] * scored.loc[both, "matchup_defender_playernorm_allowed_d10_per60"]) ** 0.5
    ...
    summary = {
        ...
        "coverage": {
            ...
            "rows_with_matchup_rate": int(pd.to_numeric(scored["matchup_defender_playernorm_allowed_d10_per60"], errors="coerce").notna().sum()),
