#!/usr/bin/env python3
"""Evaluate an exact blocked-attempt position defense base against the NHL SOG Poisson offense base."""
...
def _build_maps(raw: pd.DataFrame, blocked: pd.DataFrame) -> tuple[
    ...
    return allowed_map, blocked_all_map, blocked_d_map, float(league_all_through), float(league_d_through)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Evaluate an exact blocked-attempt position defense base against the NHL SOG Poisson offense base.")
    ...
    allowed_map, blocked_all_map, blocked_d_map, league_all_through, league_d_through = _build_maps(raw, blocked)
    ...
    for suffix, rate_col in [
        ...
        def_lam = f"lambda_defense_pos" if suffix == "plain" else f"lambda_defense_pos_{suffix}"
