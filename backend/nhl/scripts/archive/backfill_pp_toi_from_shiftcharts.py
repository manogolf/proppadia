...
def _extract_any_player_ids(d: dict) -> list[int]:
    """
    Collect candidate playerIds from varied NHL PBP shapes:
      - any int-like field whose name contains both 'player' and 'id'
      - nested arrays: details.players / details.participants with playerId
    """
    ids: list[int] = []
    if not isinstance(d, dict):
        return ids

    # flat fields (e.g., committedByPlayerId, playerId, offendingPlayerId, etc.)
    for k, v in d.items():
        kn = (k or "").lower()
        if "player" in kn and "id" in kn:
            try:
                ids.append(int(v))
            except Exception:
                pass
    ...
    # nested arrays
    for arr_key in ("players", "participants"):
        arr = d.get(arr_key)
