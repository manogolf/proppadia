#!/usr/bin/env python3
"""Evaluate exact blocked-attempt + starter-goalie overall suppression against the NHL SOG Poisson offense base."""
...
def _goalie_overall_delta_factor(goalie_allowed_per60: pd.Series, team_allowed_per60: pd.Series, start_prob: pd.Series) -> pd.Series:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Evaluate exact blocked-attempt + starter-goalie overall suppression against the NHL SOG Poisson offense base.")
    ...
    scored["goalie_allowed_d10_per60_overall"] = scored.apply(
        lambda row: goalie_allowed_overall_map.get((int(row["game_id"]), int(row["projected_goalie_id"])), math.nan)
