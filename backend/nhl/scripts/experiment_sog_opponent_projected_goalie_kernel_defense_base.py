#!/usr/bin/env python3
"""Evaluate opponent+projected-goalie conditioned continuous defense using kernel-weighted similar players."""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Evaluate opponent+projected-goalie conditioned continuous defense using kernel-weighted similar players."
    )
    ...
    for row in scored.itertuples(index=False):
        ...
        if pd.notna(row.projected_goalie_id):
            goalie_rate = _estimate_rate(
                goalie_rows.get((int(row.opponent_id), int(row.projected_goalie_id), pos_bucket), []),
                lam,
    ...
    scored["projected_goalie_kernel_allowed_rate_per60"] = goalie_rates
    ...
    for rate_col, lam_col in [
        ...
        ("projected_goalie_kernel_allowed_rate_per60", "lambda_defense_goalie_kernel"),
    ...
    summary = {
        ...
        "coverage": {
            ...
            "rows_with_goalie_rate": int(pd.to_numeric(scored["projected_goalie_kernel_allowed_rate_per60"], errors="coerce").notna().sum()),
            ...
            "avg_goalie_rate": _round(pd.to_numeric(scored["projected_goalie_kernel_allowed_rate_per60"], errors="coerce").mean()),
