#!/usr/bin/env python3
"""Evaluate exact blocked-attempt + actual-starter goalie overall suppression against the NHL SOG Poisson offense base."""
...
def _build_maps(
    ...
    allowed_overall = (
        raw.groupby(["game_date", "game_id", "opponent_id"], as_index=False)
        .agg(
            sog_allowed=("shots_on_goal", "sum"),
            toi_faced=("toi_minutes", "sum"),
        )
    ...
    team_allowed_overall_map: Dict[Tuple[int, int], float] = {}
    goalie_allowed_overall_map: Dict[Tuple[int, int], float] = {}
    ...
    for team_id, grp in allowed_overall.sort_values(["defending_team_id", "game_date", "game_id"]).groupby("defending_team_id", sort=False):
        last10: Deque[Tuple[float, float]] = deque(maxlen=10)
        for row in grp.itertuples(index=False):
            key = (int(row.game_id), int(team_id))
            if last10:
                sog_sum = sum(v[0] for v in last10)
                toi_sum = sum(v[1] for v in last10)
                team_allowed_overall_map[key] = (sog_sum * 60.0 / toi_sum) if toi_sum > 0 else math.nan
            else:
                team_allowed_overall_map[key] = math.nan
            last10.append((float(row.sog_allowed or 0.0), float(row.toi_faced or 0.0)))

    starts = starts.sort_values(["goalie_id", "game_date", "game_id"])
    for row in starts.itertuples(index=False):
        actual_starter_map[(int(row.game_id), int(row.defending_team_id))] = int(row.goalie_id)

    goalie_frame = (
        starts.merge(
            allowed_overall[["game_id", "defending_team_id", "sog_allowed", "toi_faced"]],
            on=["game_id", "defending_team_id"],
            how="left",
        )
        .sort_values(["goalie_id", "game_date", "game_id"])
    )
    for goalie_id, grp in goalie_frame.groupby("goalie_id", sort=False):
        last10: Deque[Tuple[float, float]] = deque(maxlen=10)
        for row in grp.itertuples(index=False):
            key = (int(row.game_id), int(goalie_id))
            if last10:
                sog_sum = sum(v[0] for v in last10)
                toi_sum = sum(v[1] for v in last10)
                goalie_allowed_overall_map[key] = (sog_sum * 60.0 / toi_sum) if toi_sum > 0 else math.nan
            else:
                goalie_allowed_overall_map[key] = math.nan
            last10.append((float(row.sog_allowed or 0.0), float(row.toi_faced or 0.0)))
    ...
    return (
        ...
        blocked_all_map,
        blocked_d_map,
        team_allowed_overall_map,
        goalie_allowed_overall_map,
        actual_starter_map,
        float(league_all_through),
        float(league_d_through),
    )
...
def _goalie_overall_delta_factor(goalie_allowed_per60: pd.Series, team_allowed_per60: pd.Series) -> pd.Series:
    g = pd.to_numeric(goalie_allowed_per60, errors="coerce")
    t = pd.to_numeric(team_allowed_per60, errors="coerce")
    ratio = (g / t).where(g.notna() & t.notna() & (t > 0), other=1.0)
    return ratio.clip(lower=0.75, upper=1.25)


def main() -> None:
    ap = argparse.ArgumentParser(description="Evaluate exact blocked-attempt + actual-starter goalie overall suppression against the NHL SOG Poisson offense base.")
    ...
    scored["opp_team_allowed_d10_per60_overall"] = scored.apply(
        lambda row: team_allowed_overall_map.get((int(row["game_id"]), int(row["opponent_id"])), math.nan),
        axis=1,
    )
    ...
    scored["goalie_allowed_d10_per60_overall"] = scored.apply(
        lambda row: goalie_allowed_overall_map.get((int(row["game_id"]), int(row["actual_goalie_id"])), math.nan)
    ...
    scored["goalie_overall_delta_factor"] = _goalie_overall_delta_factor(
        scored["goalie_allowed_d10_per60_overall"],
        scored["opp_team_allowed_d10_per60_overall"],
    )

    scored["rate_defense_pos"] = pd.to_numeric(scored["opp_pos_allowed_d10_per60"], errors="coerce").clip(lower=0.0)
    scored["rate_defense_pos_goalie"] = (scored["rate_defense_pos"] * scored["goalie_overall_delta_factor"]).clip(lower=0.0)
    scored["rate_defense_pos_allblocks_goalie"] = (scored["rate_defense_pos"] * scored["all_blockthrough_factor"] * scored["goalie_overall_delta_factor"]).clip(lower=0.0)
    scored["rate_defense_pos_dblocks_goalie"] = (scored["rate_defense_pos"] * scored["d_blockthrough_factor"] * scored["goalie_overall_delta_factor"]).clip(lower=0.0)
    ...
    out: Dict[str, Any] = {
        ...
        "coverage": {
            ...
            "rows_with_goalie_overall_allowed_per60": int(scored["goalie_allowed_d10_per60_overall"].notna().sum()),
            "rows_with_actual_goalie": int(pd.to_numeric(scored["actual_goalie_id"], errors="coerce").notna().sum()),
            "rows_with_team_overall_allowed_per60": int(scored["opp_team_allowed_d10_per60_overall"].notna().sum()),
