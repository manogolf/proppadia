#!/usr/bin/env python3
"""Evaluate exact blocked-attempt + actual-starter goalie position suppression against the NHL SOG Poisson offense base."""
...
def _build_maps(
    ...
    team_pos_allowed_map: Dict[Tuple[int, int, str], float] = {}
    goalie_pos_allowed_map: Dict[Tuple[int, int, str], float] = {}
    ...
    # Team baseline by position.
    for (team_id, shooter_pos), grp in allowed.sort_values(
        ["defending_team_id", "shooter_position_bucket", "game_date", "game_id"]
    ).groupby(["defending_team_id", "shooter_position_bucket"], sort=False):
        ...
        for row in grp.itertuples(index=False):
            ...
            if last10:
                ...
                team_pos_allowed_map[key] = (sog_sum * 60.0 / toi_sum) if toi_sum > 0 else math.nan
            else:
                team_pos_allowed_map[key] = math.nan
            last10.append((float(row.pos_sog_allowed or 0.0), float(row.pos_toi_faced or 0.0)))

    # Actual starter goalie allowed by shooter position.
    goalie_frame = (
        starts.merge(
    ...
    for (goalie_id, shooter_pos), grp in goalie_frame.groupby(["goalie_id", "shooter_position_bucket"], sort=False):
        ...
        for row in grp.itertuples(index=False):
            ...
            if last10:
                sog_sum = sum(v[0] for v in last10)
                toi_sum = sum(v[1] for v in last10)
                goalie_pos_allowed_map[key] = (sog_sum * 60.0 / toi_sum) if toi_sum > 0 else math.nan
            else:
                goalie_pos_allowed_map[key] = math.nan
            last10.append((float(row.pos_sog_allowed or 0.0), float(row.pos_toi_faced or 0.0)))
    ...
    return (
        ...
        blocked_all_map,
        blocked_d_map,
        team_pos_allowed_map,
        goalie_pos_allowed_map,
        actual_starter_map,
...
def _goalie_pos_delta_factor(goalie_allowed_per60: pd.Series, team_allowed_per60: pd.Series) -> pd.Series:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Evaluate exact blocked-attempt + actual-starter goalie position suppression against the NHL SOG Poisson offense base.")
    ...
    scored["opp_team_pos_allowed_d10_per60"] = scored.apply(
        lambda row: team_pos_allowed_map.get((int(row["game_id"]), int(row["opponent_id"]), str(row["position_bucket"])), math.nan),
        axis=1,
    )
    ...
    scored["goalie_allowed_d10_per60_pos"] = scored.apply(
        lambda row: goalie_pos_allowed_map.get((int(row["game_id"]), int(row["actual_goalie_id"]), str(row["position_bucket"])), math.nan)
    ...
    scored["goalie_pos_delta_factor"] = _goalie_pos_delta_factor(
        scored["goalie_allowed_d10_per60_pos"],
        scored["opp_team_pos_allowed_d10_per60"],
    )

    scored["rate_defense_pos"] = pd.to_numeric(scored["opp_pos_allowed_d10_per60"], errors="coerce").clip(lower=0.0)
    scored["rate_defense_pos_goalie"] = (scored["rate_defense_pos"] * scored["goalie_pos_delta_factor"]).clip(lower=0.0)
    scored["rate_defense_pos_allblocks_goalie"] = (
        scored["rate_defense_pos"] * scored["all_blockthrough_factor"] * scored["goalie_pos_delta_factor"]
    ).clip(lower=0.0)
    scored["rate_defense_pos_dblocks_goalie"] = (
        scored["rate_defense_pos"] * scored["d_blockthrough_factor"] * scored["goalie_pos_delta_factor"]
    ).clip(lower=0.0)
    ...
    out: Dict[str, Any] = {
        ...
        "coverage": {
            ...
            "rows_with_goalie_pos_allowed_per60": int(scored["goalie_allowed_d10_per60_pos"].notna().sum()),
            "rows_with_actual_goalie": int(pd.to_numeric(scored["actual_goalie_id"], errors="coerce").notna().sum()),
            "rows_with_team_pos_allowed_per60": int(scored["opp_team_pos_allowed_d10_per60"].notna().sum()),
