-- Drops legacy MLB relations that were moved from public to mlb in phase 2.
-- Keeps core runtime tables intact (phase-1 set).
...
BEGIN
    FOREACH object_name IN ARRAY object_names LOOP
        ...
        IF object_kind = 'r' THEN
            EXECUTE format('DROP TABLE IF EXISTS mlb.%I CASCADE', object_name);
        ELSIF object_kind = 'm' THEN
            EXECUTE format('DROP MATERIALIZED VIEW IF EXISTS mlb.%I CASCADE', object_name);
        ELSIF object_kind = 'v' THEN
            EXECUTE format('DROP VIEW IF EXISTS mlb.%I CASCADE', object_name);
        ELSE
            RAISE EXCEPTION 'Unsupported relkind % for mlb.%', object_kind, object_name;
        END IF;

        RAISE NOTICE 'Dropped mlb.% (%)', object_name, object_kind;
    END LOOP;
END
$$;
