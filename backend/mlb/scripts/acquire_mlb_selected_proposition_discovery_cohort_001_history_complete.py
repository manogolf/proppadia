#!/usr/bin/env python3
"""Acquire exact historical evidence for DISCOVERY_COHORT_001.
...
EXPECTED_REQUEST_MANIFEST_SHA = "36482982401b41c18f0856ef33b0c2033cb85f174732c92517a9bfe60e6a1c2c"

GOV_STATUS = (
    "STARTER_DISCOVERY_COHORT_001_ACQUISITION_GOVERNANCE_STATUS = "
...
DISCOVERY_REQUESTS = DISCOVERY_DIR / f"proposed_exact_acquisition_manifest_{RUN_DATE}.csv"
DISCOVERY_SIDES = DISCOVERY_DIR / f"side_level_discovery_result_ledger_{RUN_DATE}.csv"
GOV_SIDES = GOVERNANCE_DIR / f"first_discovery_cohort_side_manifest_{RUN_DATE}.csv"
GOV_ROWS = GOVERNANCE_DIR / f"first_discovery_cohort_row_manifest_{RUN_DATE}.csv"
GOV_TARGETS = GOVERNANCE_DIR / f"first_discovery_cohort_target_manifest_{RUN_DATE}.csv"
...
class DiscoveryCohortHistoryAcquisition:
    ...
    def build_executable_manifest(self) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        for order, key in enumerate(sorted(grouped), start=1):
            ...
            executable_rows.append({
                ...
                "source_class_or_endpoint": first["source_class_or_endpoint"],
                "method": "GET",
                ...
                "historical_game_date": first["historical_game_date"],
                ...
                "request_purpose": first["expected_record_purpose"],
            ...
            for original in originals:
                mapping_rows.append({
                    ...
                    "historical_game_date": original["historical_game_date"],
    ...
    def verify_inputs(self, executable: list[dict[str, Any]], original_manifest: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        row_side_keys = {r["starter_game_key"] for r in self.gov_rows}
        ...
        checks = [
            ...
            ("exact_10_side_reproduction", len(self.gov_sides), 10),
            ("exact_98_row_reproduction", len(self.gov_rows), 98),
            ("exact_10_target_reproduction", len(self.gov_targets), 10),
            ("exact_239_original_request_reproduction", len(self.requests), 239),
            ("request_identity_uniqueness", len(original_request_ids), 239),
            ...
            ("no_manifest_expansion", len(original_manifest), 239),
    ...
    def write_governance_first(self, executable: list[dict[str, Any]], mapping: list[dict[str, Any]], original_manifest: list[dict[str, Any]], validations: list[dict[str, Any]]) -> None:
        ...
        write_csv(OUT_DIR / f"exact_original_239_request_manifest_{RUN_DATE}.csv", original_manifest)
    ...
    def fetch_one(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(1, self.retry_limit + 2):
            ...
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "proppadia-bounded-cohort001-acquisition/1.0"})
    ...
    def parse_one(self, request: dict[str, Any], response: dict[str, Any]) -> dict[str, Any]:
        ...
        if response["final_transport_result"] != "SUCCESS" or not response.get("raw_response_path"):
            return {**base, "rejection_taxonomy": "REQUEST_EXECUTION_FAILURE"}
        try:
            payload = json.loads(Path(response["raw_response_path"]).read_text(encoding="utf-8"))
        except Exception as exc:
            return {**base, "rejection_taxonomy": "PARSER_FAILURE", "parse_error": str(exc)}
        ...
        if not found_player:
            record = {
                **base,
                ...
                "rejection_taxonomy": "PITCHER_IDENTITY_FAILURE",
            }
        ...
        if not game_ok:
            reject = "GAME_IDENTITY_FAILURE"
        elif not temporal_ok:
            reject = "TEMPORAL_FAILURE"
        elif not role_ok:
            reject = "ROLE_REGIME_FAILURE"
        elif not workload_ok:
            reject = "WORKLOAD_OR_SOURCE_FACT_INCOMPLETE"
        else:
            reject = ""
    ...
    def write_side_outputs(self, executable: list[dict[str, Any]], parsed_records: list[dict[str, Any]]) -> None:
        ...
        for side in sorted(side_meta):
            ...
            side_rows.append({
                ...
                "projected_qualification_ceiling": meta.get("projected_fully_qualified_ceiling", ""),
    ...
    def write_projected_ceilings(self, parsed_records: list[dict[str, Any]]) -> None:
        ...
        rows_complete = [r for r in self.gov_rows if r["starter_game_key"] in complete]
        rows_partial = [r for r in self.gov_rows if r["starter_game_key"] in partial]
        write_csv(OUT_DIR / f"projected_reconstruction_ceilings_{RUN_DATE}.csv", [{
            ...
            "projected_full_qualification_ceiling_for_history_complete_sides": len(rows_complete),
            ...
            "potential_abd_matrix_readiness_additions": sum(r.get("existing_abd_matrix_overlap") == "false" and r.get("line") == "1.5" for r in rows_complete),
...
# Discovery Cohort 001 History-Complete Acquisition — {RUN_DATE}
...
def main() -> int:
    ...
    runner = DiscoveryCohortHistoryAcquisition(
        mode=args.mode,
