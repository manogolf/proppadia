#!/usr/bin/env python3
"""Execute bounded offline Starter remediation for DISCOVERY_COHORT_001.

This is a deterministic offline research overlay. It consumes only the frozen
DISCOVERY_COHORT_001 reconstruction governance package, the preserved certified
...
EXPECTED_GOV_SHA = "b4142267918d8a03d3f79bb586c0e6e6f1669db74499226d5ce582f186d2a720"
EXPECTED_ACQ_SHA = "1392d7397bcb05642a5af034ca15bcbf580af76bee989c39a8a346a8010fb9e1"
...
DECISION_VALIDATED = (
    "STARTER_DISCOVERY_COHORT_001_RECONSTRUCTION_REMEDIATION_DECISION = "
    "DISCOVERY_TO_ACQUISITION_TO_REMEDIATION_PIPELINE_VALIDATED_SCALE_UP_RECOMMENDED"
)
DECISION_PARTIAL = (
    "STARTER_DISCOVERY_COHORT_001_RECONSTRUCTION_REMEDIATION_DECISION = "
...
DECISION_LOW = (
    "STARTER_DISCOVERY_COHORT_001_RECONSTRUCTION_REMEDIATION_DECISION = "
    "REMEDIATION_YIELD_INSUFFICIENT_NO_SCALE_UP"
)

REC_SCALE = "DISCOVERY_TO_ACQUISITION_TO_REMEDIATION_PIPELINE_VALIDATED_SCALE_UP_RECOMMENDED"
...
REC_LOW = "REMEDIATION_YIELD_INSUFFICIENT_NO_SCALE_UP"
...
class DiscoveryCohortStarterRemediation:
    ...
    def verify(self) -> list[dict[str, Any]]:
        ...
        checks = [
            ...
            ("exact_239_record_reproduction", len(self.records), 239),
            ...
            ("all_98_rows_accounted_for", len(row_ids), 98),
            ...
            ("all_source_records_certified", sum(r["accepted_rejected_state"] == "ACCEPTED_FULLY_CERTIFIED_SOURCE_RECORD" for r in self.records), 239),
            ("all_governed_rows_in_authoritative_state", len({row["governed_canonical_row_id"] for row in self.rows} & {row["governed_canonical_row_id"] for row in self.state_ledger}), 98),
    ...
    def state_payload(self, movement: list[dict[str, Any]], side_results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            "decision": DECISION_VALIDATED if len(fully) == 98 and all(s["certification_result"] == "STARTER_SIDE_CERTIFIED" for s in side_results) else DECISION_PARTIAL,
            "recommendation": REC_SCALE if len(fully) == 98 else REC_PARTIAL,
            ...
            "current_pa_blocked_population": baseline["pa_blocked_rows"],
            "current_outcome_blocked_population": baseline["outcome_blocked_rows"],
            "current_bundle_blocked_population": baseline["bundle_field_blocked_rows"],
            ...
            "realized_starter_qualification_yield_against_98_row_ceiling": round(len(starter_qualified) / 98, 6),
            "realized_full_qualification_yield_against_98_row_ceiling": round(len(fully) / 98, 6),
            ...
            "projected_vs_realized": {
                "projected_starter_qualified_ceiling": 98,
                "realized_starter_qualified": len(starter_qualified),
                "projected_newly_fully_qualified_ceiling": 98,
                "realized_newly_fully_qualified": len(fully),
                "projected_hits_0_5_additions": 89,
                "realized_hits_0_5_additions": hits_05,
                "projected_hits_1_5_additions": 9,
                "realized_hits_1_5_additions": hits_15,
                "projected_abd_matrix_readiness_additions": 9,
    ...
    def run(self) -> dict[str, Any]:
        ...
        validation.extend([
            ...
            {"validation": "all_98_rows_accounted_for_in_movement_ledger", "status": "PASS" if len(movement) == 98 else "FAIL", "observed": len(movement), "expected": 98},
        ])
        ...
        write_csv(OUT_DIR / f"deterministic_replay_report_{RUN_DATE}.csv", [
            ...
            {"check": "bounded_overlay", "status": "PASS", "notes": "Exact 10 sides and 98 rows only."},
        ])
...
# DISCOVERY_COHORT_001 Starter Remediation Certified State — {RUN_DATE}
...
# DISCOVERY_COHORT_001 Starter Reconstruction/Remediation Execution — {RUN_DATE}

Decision: `{payload['decision']}`

The execution consumed only the frozen governance package, the preserved 239
certified strict-prior source records, and the existing certified campaign state.
All 10 governed Starter-game sides certified. Starter qualification propagated
only to the exact 98 governed denominator rows, and all 98 became newly fully
qualified in this overlay.
...
def main() -> int:
    result = DiscoveryCohortStarterRemediation().run()
