#!/usr/bin/env python3
"""Execute bounded discovery for frozen DISCOVERY_COHORT_002.
...
EXPECTED_SOURCE_PACKAGE_HASHES = {
...
TARGET_MANIFEST = SCALE_UP_DIR / f"discovery_cohort_002_target_manifest_{RUN_DATE}.csv"
SIDE_MANIFEST = SCALE_UP_DIR / f"discovery_cohort_002_side_manifest_{RUN_DATE}.csv"
ROW_MANIFEST = SCALE_UP_DIR / f"discovery_cohort_002_row_manifest_{RUN_DATE}.csv"
SCALE_UP_STATE = SCALE_UP_DIR / f"machine_readable_scale_up_design_{RUN_DATE}.json"

PACKAGE_MANIFESTS = {
    "scale_up_design": SCALE_UP_DIR / f"sha256_manifest_{RUN_DATE}.csv",
    "original_803_readiness": Path(
        "artifacts/analysis/model_development/"
        "mlb_historical_selected_proposition_803_starter_direct_source_recovery_readiness_review/"
        "2026-07-14/sha256_manifest_2026-07-14.csv"
    ),
    "prescreen_discovery_governance": Path(
        "artifacts/analysis/model_development/"
        "mlb_historical_selected_proposition_starter_prescreen_and_discovery_cohort_governance/"
        "2026-07-15/sha256_manifest_2026-07-15.csv"
    ),
    "four_side_history_complete_remediation": Path(
        "artifacts/analysis/model_development/"
        "mlb_historical_selected_proposition_four_side_history_complete_starter_reconstruction_remediation/"
        "2026-07-15/sha256_manifest_2026-07-15.csv"
    ),
    "hc_local_cohort_001_remediation": Path(
        "artifacts/analysis/model_development/"
        "mlb_historical_selected_proposition_hc_local_cohort_001_starter_reconstruction_remediation/"
        "2026-07-15/sha256_manifest_2026-07-15.csv"
    ),
    "discovery_cohort_001_remediation": Path(
        "artifacts/analysis/model_development/"
        "mlb_historical_selected_proposition_discovery_cohort_001_starter_reconstruction_remediation/"
        "2026-07-15/sha256_manifest_2026-07-15.csv"
    ),
    "post_three_row_state_baseline": Path(
        "artifacts/analysis/model_development/"
...
class DiscoveryCohort002:
    def __init__(self, allow_network: bool, timeout: int) -> None:
        self.allow_network = allow_network
        self.timeout = timeout
        self.scale_up_state = json.loads(SCALE_UP_STATE.read_text(encoding="utf-8"))
    ...
    def verify_inputs(self) -> list[dict[str, Any]]:
        ...
        validations = []
        actual_scale_up_sha = package_sha(PACKAGE_MANIFESTS["scale_up_design"])
        validations.append({
            "validation": "scale_up_design_package_sha",
            "status": "PASS" if actual_scale_up_sha == EXPECTED_SCALE_UP_SHA else "FAIL",
            "observed": actual_scale_up_sha,
            "expected": EXPECTED_SCALE_UP_SHA,
        })
        for package_name, expected in EXPECTED_SOURCE_PACKAGE_HASHES.items():
            actual = package_sha(PACKAGE_MANIFESTS[package_name])
            validations.append({
                "validation": f"authoritative_source_package_sha_{package_name}",
                "status": "PASS" if actual == expected else "FAIL",
                "observed": actual,
                "expected": expected,
            })
        checks = [
            ("governance_status", self.scale_up_state.get("cohort_002_governance_status"), "STARTER_DISCOVERY_COHORT_002_GOVERNANCE_STATUS = FROZEN_AWAITING_EXPLICIT_BOUNDED_DISCOVERY_APPROVAL"),
    ...
    def run(self) -> dict[str, Any]:
        ...
        for target in sorted(self.targets, key=lambda r: int_value(r["target_order"])):
            ...
            game_id = target["governed_target_game"]
            governed_date = target["governed_target_date"]
            feed_request_id = f"DISCOVERY_COHORT_002|{safe_token(side_key)}|target_game_feed"
            ...
            if accepted_pitcher:
                for season in sorted({str(parse_date(governed_date).year - 1), str(parse_date(governed_date).year)}):
                    log_request_id = (
                        f"DISCOVERY_COHORT_002|{safe_token(side_key)}|"
            ...
            side_ledgers.append({
                ...
                "frozen_discovery_classification": side_row["discovery_classification"],
                ...
                "game_history_candidates": "|".join(f"{r['historical_game_date']}:{r['historical_game_id']}" for r in history_records),
        ...
        for rec in accepted_records:
            ...
            row = {
                "acquisition_request_id": f"DISCOVERY_COHORT_002_ACQ|{safe_token(dedupe)}",
        ...
        if len(resolved_side_keys) == len(self.targets):
            ...
            decision = STARTER_DISCOVERY_COHORT_002_DECISION_READY
        elif resolved_side_keys:
            recommendation = RECOMMEND_PARTIAL
            decision = STARTER_DISCOVERY_COHORT_002_DECISION_PARTIAL
        elif ambiguity_count:
            recommendation = RECOMMEND_AMBIGUITY
            decision = STARTER_DISCOVERY_COHORT_002_DECISION_AMBIGUITY
        elif raw_inventory and any(r["retrieval_status"] == "SUCCESS" for r in raw_inventory):
            recommendation = RECOMMEND_LOW
            decision = STARTER_DISCOVERY_COHORT_002_DECISION_LOW
        else:
            recommendation = RECOMMEND_FAILED
            decision = STARTER_DISCOVERY_COHORT_002_DECISION_FAILED
        ...
        write_csv(OUT_DIR / f"acquisition_manifest_deduplication_report_{RUN_DATE}.csv", [{
            ...
            "frozen_estimated_later_historical_acquisition_requests": self.scale_up_state.get("cohort_002_estimated_later_acquisition_request_count"),
            "difference_vs_frozen_estimate": len(manifest_rows) - int_value(self.scale_up_state.get("cohort_002_estimated_later_acquisition_request_count")),
        ...
        write_csv(OUT_DIR / f"projected_reconstruction_ceilings_{RUN_DATE}.csv", [{
            ...
            "potential_abd_matrix_readiness_additions": sum(int_value(s["potential_abd_matrix_readiness_additions"]) for s in resolved_sides),
        ...
        replay_rows = [
            ...
            {"check": "bounded_targets", "status": "PASS", "notes": "exact frozen COHORT_002 target manifest only"},
    ...
    def render_summary(self, payload: dict[str, Any]) -> str:
        return f"""
# Discovery Cohort 002 Execution — {RUN_DATE}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Execute bounded DISCOVERY_COHORT_002 discovery.")
    parser.add_argument("--mode", choices=["execute", "replay"], default="execute")
    parser.add_argument("--timeout-seconds", type=int, default=30)
    args = parser.parse_args()
    result = DiscoveryCohort002(allow_network=args.mode == "execute", timeout=args.timeout_seconds).run()
