#!/usr/bin/env python3
"""Acquire exact historical evidence for DISCOVERY_COHORT_002.
...
EXPECTED_REQUEST_MANIFEST_SHA = "d359cdf828355208c9488a7652277f87647bd5e83aa323ef61916a669ed888ec"

GOV_STATUS = (
    "STARTER_DISCOVERY_COHORT_002_ACQUISITION_GOVERNANCE_STATUS = "
...
GOV_SIDES = GOVERNANCE_DIR / f"discovery_cohort_002_side_manifest_{RUN_DATE}.csv"
GOV_ROWS = GOVERNANCE_DIR / f"discovery_cohort_002_row_manifest_{RUN_DATE}.csv"
GOV_TARGETS = GOVERNANCE_DIR / f"discovery_cohort_002_target_manifest_{RUN_DATE}.csv"
...
class DiscoveryCohort002HistoryAcquisition:
    ...
    def verify_inputs(self, executable: list[dict[str, Any]], original_manifest: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        checks = [
            ...
            ("exact_76_row_reproduction", len(self.gov_rows), 76),
            ("exact_8_target_reproduction", len(self.gov_targets), 8),
            ("exact_139_original_request_reproduction", len(self.requests), 139),
            ("request_identity_uniqueness", len(original_request_ids), 139),
            ...
            ("no_manifest_expansion", len(original_manifest), 139),
    ...
    def write_governance_first(self, executable: list[dict[str, Any]], mapping: list[dict[str, Any]], original_manifest: list[dict[str, Any]], validations: list[dict[str, Any]]) -> None:
        ...
        write_csv(OUT_DIR / f"exact_original_139_request_manifest_{RUN_DATE}.csv", original_manifest)
    ...
    def fetch_one(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(1, self.retry_limit + 2):
            ...
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "proppadia-bounded-cohort002-acquisition/1.0"})
    ...
    def write_side_outputs(self, executable: list[dict[str, Any]], parsed_records: list[dict[str, Any]]) -> None:
        ...
        for side in sorted(side_meta):
            ...
            side_rows.append({
                ...
                "projected_qualification_ceiling": meta.get("projected_fully_qualified_ceiling") or meta.get("projected_newly_fully_qualified_ceiling", ""),
    ...
    def write_projected_ceilings(self, parsed_records: list[dict[str, Any]]) -> None:
        ...
        write_csv(OUT_DIR / f"projected_reconstruction_ceilings_{RUN_DATE}.csv", [{
            ...
            "potential_abd_matrix_readiness_additions": sum(
                int_value(side.get("potential_abd_matrix_readiness_additions") or side.get("expected_abd_readiness_additions"))
                for side in self.gov_sides
                if side["starter_game_side_key"] in complete
            ),
...
# Discovery Cohort 002 History-Complete Acquisition — {RUN_DATE}
...
def main() -> int:
    ...
    runner = DiscoveryCohort002HistoryAcquisition(
        mode=args.mode,
