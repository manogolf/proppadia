#!/usr/bin/env python3
"""Freeze Starter reconstruction/remediation governance for DISCOVERY_COHORT_002.

Governance only. This utility consumes the completed DISCOVERY_COHORT_002
...
COHORT_ID = "DISCOVERY_COHORT_002"
...
EXPECTED_ACQUISITION_SHA = "2b4d03d6d501a25db8b4a338f738e1f02234c3ac314d49f268f717931f78275d"
...
EXPECTED_ACQUISITION_DECISION = (
    "STARTER_DISCOVERY_COHORT_002_HISTORY_COMPLETE_ACQUISITION_DECISION = "
...
EXPECTED_DISCOVERY_DECISION = (
    "STARTER_DISCOVERY_COHORT_002_DECISION = "
...
READINESS_DIR = Path(
    "artifacts/analysis/model_development/"
    "mlb_historical_selected_proposition_803_starter_direct_source_recovery_readiness_review/2026-07-14"
)
MATRIX_DIR = Path(
    "artifacts/analysis/model_development/"
...
SCALE_UP_RESULT = SCALE_UP_DIR / f"machine_readable_scale_up_design_{RUN_DATE}.json"
COHORT_ROWS = SCALE_UP_DIR / f"discovery_cohort_002_row_manifest_{RUN_DATE}.csv"
COHORT_SIDES = SCALE_UP_DIR / f"discovery_cohort_002_side_manifest_{RUN_DATE}.csv"
ORIGINAL_803_ROWS = READINESS_DIR / "exact_803_row_denominator_manifest_2026-07-14.csv"
...
class DiscoveryCohort002StarterGovernance:
    def __init__(self) -> None:
        ...
        self.scale_up_result = json.loads(SCALE_UP_RESULT.read_text(encoding="utf-8"))
        ...
        self.scale_sides = read_csv(COHORT_SIDES)
        self.original_rows = read_csv(ORIGINAL_803_ROWS)
        self.original_by_row_id = {row_identity(row): row for row in self.original_rows}
        self.rows_by_side: dict[str, list[dict[str, str]]] = defaultdict(list)
        for row in self.rows:
            self.rows_by_side[row["starter_game_side_key"]].append(row)

    def original_row(self, row: dict[str, str]) -> dict[str, str]:
        original = self.original_by_row_id.get(row_identity(row))
        if original is None:
            raise KeyError(f"missing original 803-row denominator identity: {row_identity(row)}")
        return original

    def downstream_blockers_for(self, row: dict[str, str]) -> list[str]:
        original = self.original_row(row)
        blockers = []
        if not bool_text(original.get("post_three_row_pa_qualified", "")):
            blockers.append("PA")
        if original.get("outcome_category") != "NUMERIC_OUTCOME_CERTIFIED":
            blockers.append("OUTCOME")
        if original.get("post_three_row_downstream_blockers"):
            blockers.append(original["post_three_row_downstream_blockers"])
        return blockers
    ...
    def verify(self) -> list[dict[str, Any]]:
        ...
        scale_side_keys = {row["starter_game_side_key"] for row in self.scale_sides}
        record_sides = set()
        ...
        missing_original = [rid for rid in row_ids if rid not in self.original_by_row_id]
        checks = [
            ...
            ("scale_up_package_sha_verification", package_sha(SCALE_UP_DIR), EXPECTED_SCALE_UP_SHA),
            ...
            ("exact_139_record_reproduction", len(self.acq_parsed), 139),
            ("exact_139_request_reproduction", len(self.acq_requests), 139),
            ...
            ("scale_side_manifest_binding", sorted(scale_side_keys), sorted(side_keys)),
            ("row_identity_uniqueness", len(row_ids), 76),
            ("original_803_row_binding", missing_original, []),
            ("all_source_records_certified", sum(r["accepted_rejected_state"] == "ACCEPTED_FULLY_CERTIFIED_SOURCE_RECORD" for r in self.acq_parsed), 139),
            ("all_sides_history_complete", sum(r["complete_partial_failed_status"] == "HISTORY_COMPLETE" for r in self.acq_sides), 8),
            ("represented_denominator_rows_supported", int_value(self.acq_result.get("represented_denominator_rows_supported_by_complete_sides")), 76),
            ("projected_full_qualification_ceiling", int_value(self.acq_result.get("projected_full_qualification_ceiling_for_history_complete_sides")), 73),
            ("potential_abd_additions", int_value(self.acq_result.get("potential_abd_matrix_readiness_additions")), 3),
    ...
    def governed_side_manifest(self) -> list[dict[str, Any]]:
        ...
        scale_by_side = {row["starter_game_side_key"]: row for row in self.scale_sides}
        ...
        for side in sorted(self.acq_sides, key=lambda r: r["starter_game_side_identity"]):
            ...
            rows.append({
                ...
                "projected_starter_qualified_ceiling": side["represented_row_count"],
                "projected_newly_fully_qualified_ceiling": scale.get("projected_newly_fully_qualified_ceiling", side["projected_qualification_ceiling"]),
                ...
                "potential_abd_matrix_readiness_additions": scale.get("potential_abd_matrix_readiness_additions", ""),
                "variant_c_implication": scale.get("variant_c_implication", "governance_preserved_not_resolved"),
    ...
    def row_manifest(self) -> list[dict[str, Any]]:
        ...
        for row in sorted(self.rows, key=row_identity):
            ...
            matrix_addition = (
                row["line"] == "1.5"
                and original.get("existing_abd_matrix_overlap") == "false"
                and fully_qualified
            )
            rows.append({
                "cohort_id": COHORT_ID,
                ...
                "line": row["line"],
                "side": row["side"],
                "pre_remediation_starter_status": original["post_three_row_primary_classification"],
                "pre_remediation_starter_qualified": original["post_starter_workload_starter_qualified"],
                "pa_qualified_current": original["post_three_row_pa_qualified"],
                "pa_status_current": original["post_three_row_pa_status"],
                "outcome_certified_current": original["numeric_outcome_certified"],
                "outcome_category_current": original["outcome_category"],
                "bundle_blockers_current": original.get("post_three_row_downstream_blockers", ""),
                "existing_abd_matrix_overlap": original["existing_abd_matrix_overlap"],
                "variant_a_state_current": original["post_three_row_variant_a_state"],
                "variant_b_state_current": original["post_three_row_variant_b_state"],
                "variant_c_state_current": original["post_three_row_variant_c_state"],
                "variant_d_state_current": original["post_three_row_variant_d_state"],
    ...
    def formula_contract(self) -> list[dict[str, Any]]:
        ...
        for domain in PARENT_DOMAINS:
            if domain == "authoritative_actual_starter_identity":
                ...
                source = "139 certified source records plus admitted local parent artifacts"
    ...
    def overlay_contract(self) -> list[dict[str, Any]]:
        return [
            ...
            {"contract_item": "scale_up_design_package", "frozen_rule": "remain unchanged", "execution_requirement": "read-only input"},
    ...
    def approval_boundary(self) -> list[dict[str, Any]]:
        return [
            {"approval_boundary_item": "authorized_by_later_user_approval", "value": "one deterministic offline reconstruction/remediation execution for exact 8 sides and 76 rows only"},
            {"approval_boundary_item": "allowed_sources", "value": "frozen 139 certified source records and explicitly admitted local parents only"},
    ...
    def run(self) -> dict[str, Any]:
        ...
        write_csv(OUT_DIR / f"exact_downstream_blocker_analysis_{RUN_DATE}.csv", self.downstream_blocker_analysis(row_manifest))
        ...
        validation.extend([
            ...
            {"validation": "source_artifacts_byte_identical_scale_up", "status": "PASS", "observed": package_sha(SCALE_UP_DIR), "expected": EXPECTED_SCALE_UP_SHA},
        ])
        write_csv(OUT_DIR / f"validation_report_{RUN_DATE}.csv", validation)
        payload = {
            "status": STATUS,
            ...
            "exact_separate_approval_required_next": "one deterministic offline reconstruction/remediation execution for exact 8 sides and 76 rows only",
...
# DISCOVERY_COHORT_002 Starter Reconstruction Governance — {RUN_DATE}
...
source-record bindings, formulas, fail-closed taxonomy, propagation rules, and
approval boundaries for a possible later offline Starter reconstruction pass.
It does not execute reconstruction, remediation, qualification propagation,
matrix construction, modeling, scoring, uploads, database/API writes,
...
reconstruction/remediation execution for the frozen 8 sides and 76 rows only.
The three downstream-limited rows remain PA-blocked after hypothetical Starter
success and are not allowed to be force-promoted by this governance package.
""")
        self.parse_and_hash()
...
def main() -> int:
    result = DiscoveryCohort002StarterGovernance().run()
