...
def run(game_date: str, output_dir: Path, ledger_path: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        flat.append(item)
    pd.DataFrame(flat).to_csv(output_dir/"august_6_totals_shadow_predictions.csv", index=False)
    ...
    pd.DataFrame([{key: row.get(key) for key in market_columns} for row in rows]).to_csv(output_dir/"august_6_total_market_attachment.csv", index=False)
    validation = [
    ...
    contract = {"experiment": EXPERIMENT, "model_version": MODEL_VERSION, "model_hash": candidate["canonical_model_hash"], "model_source": "MLB_TOTALS_PREDICTION_REPRESENTATIVE_RERUN_V1",
        ...
        "prospective_authority_begins": "2026-08-06", "august_6_outcomes_accessed_during_prediction": 0, "public_status": "SHADOW_ONLY_NOT_PUBLIC"}
