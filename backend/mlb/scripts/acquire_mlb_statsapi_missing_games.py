...
def main():
 ...
 u=BASE+'/api/v1/schedule?'+urlencode({'sportId':1,'startDate':a.start,'endDate':a.end,'gameType':'R','hydrate':'status'}); _,b=fetch(u,a.timeout); sched=json.loads(b); (out/'schedule_2026-03-26_2026-07-21.json').write_bytes(b); rows=[]
  ...
  for g in day.get('games',[]):
   ...
   if state=='Final':
    if gid in local:
     ...
     if not dest.exists(): dest.write_bytes(lb)
    elif dest.exists(): cls='LOCAL_CERTIFIED_REUSED'
    else:
 ...
 with (out/'completion_ledger.csv').open('w',newline='') as f:w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
