#!/usr/bin/env python3
"""
Backfill MLB player-prop odds snapshots from The Odds API historical endpoints.
...
Usage:
  python backend/mlb/scripts/backfill_mlb_oddsapi_history.py --season 2025
  python backend/mlb/scripts/backfill_mlb_oddsapi_history.py --from-date 2025-03-27 --to-date 2025-09-28
"""
...
DEFAULT_MARKETS = ",".join(sorted(set(PROP_TO_ODDS_MARKET.values())))
DEFAULT_REGIONS = "us,us2"
DEFAULT_ODDS_FORMAT = "american"
DEFAULT_SNAPSHOT_TIME_ET = "13:00"

# Season ids are start-year.
# For MLB regular season historical pull, we map to opening day.
...
DEFAULT_OUT_ROOT = BACKEND_DIR / "mlb" / "exports" / "odds_history"
...
def _chunked_market_groups(markets_csv: str, max_markets_per_call: int) -> list[str]:
    ...
    for i in range(0, len(toks), step):
        groups.append(",".join(toks[i : i + step]))
    return groups
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Backfill MLB historical player-prop odds snapshots by ET date.")
    ...
    ap.add_argument("--max-markets-per-call", type=int, default=6, help="Market chunk size per event-odds call.")
    ...
    ctx = FetchContext(
        ...
        market_groups=_chunked_market_groups(args.markets, args.max_markets_per_call),
    ...
    print(
        f"[mlb-odds-backfill] start={start_d} end={end_d} days_total={len(all_days)} "
        f"days_pending={len(pending_all)} days_run={len(run_days)} "
        f"snapshot_time_et={args.snapshot_time_et.strftime('%H:%M')}"
    )
    print(f"[mlb-odds-backfill] out_root={out_root}")
    print(f"[mlb-odds-backfill] market call groups={ctx.market_groups}")
    ...
    for d in run_days:
        ...
        if manifest_path.exists() and not args.overwrite:
            print(f"[mlb-odds-backfill] skip {day_str}: manifest exists")
            skipped += 1
            continue

        snapshot_iso = _to_utc_snapshot_iso(d, args.snapshot_time_et)
        print(f"[mlb-odds-backfill] {day_str} snapshot_utc={snapshot_iso}")
        ...
        try:
            ...
            for idx, ev in enumerate(events_for_slate, start=1):
                ...
                for group_idx, markets_csv in enumerate(ctx.market_groups, start=1):
                    try:
                        ...
                        print(
                            f"[mlb-odds-backfill] {day_str} event {event_id} "
            ...
            print(
                f"[mlb-odds-backfill] wrote {day_str}: events_for_slate={len(events_for_slate)} "
                f"odds_records={len(odds_data)} remaining={manifest['api_usage']['x-requests-remaining']}"
            )
            copied += 1
        except Exception as e:
            print(f"[mlb-odds-backfill] FAILED {day_str}: {e}")
            failed += 1
    ...
    print(
        f"[mlb-odds-backfill] done copied={copied} skipped={skipped} failed={failed} "
        f"out_root={out_root}"
    )
