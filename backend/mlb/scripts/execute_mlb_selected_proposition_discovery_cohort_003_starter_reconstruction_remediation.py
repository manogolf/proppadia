#!/usr/bin/env python3
"""Execute bounded offline Starter remediation for DISCOVERY_COHORT_003.

This is a deterministic offline research overlay. It consumes only the frozen
DISCOVERY_COHORT_003 reconstruction governance package, the preserved certified
...
EXPECTED_GOV_SHA = "812d666b744473d9f5ff6e0707aecb2f0ae7948501efa2941a29a879ba5fcc10"
EXPECTED_ACQ_SHA = "04d225bfe8a685d46bd8ab05657d291c582b1dd8854de87d07b6edb7872fee90"
...
POST_STATE_CERTIFIED = "STARTER_POST_COHORT_003_CUMULATIVE_QUALIFICATION_STATE = CERTIFIED"
...
REMAINING_DISCOVERY = CUMULATIVE_DIR / f"remaining_discovery_population_reconciliation_{RUN_DATE}.csv"
FUTURE_IMPACT = CUMULATIVE_DIR / f"future_cohort_manifest_impact_assessment_{RUN_DATE}.csv"
...
class DiscoveryCohort003StarterRemediation:
    def __init__(self) -> None:
        ...
        self.remaining_discovery = read_csv(REMAINING_DISCOVERY)
        self.future_impact = read_csv(FUTURE_IMPACT)
        ...
        self.cumulative_hash_before = package_sha(CUMULATIVE_DIR)
    ...
    def verify(self) -> list[dict[str, Any]]:
        ...
        previously_moved_ids = {row["canonical_denominator_identity"] for row in self.cumulative_ledger}
        all_rows_starter_blocked = all(
            row["pre_remediation_starter_status"] == "HITS_STARTER_BLOCKED_DIRECT_SOURCE_MISSING"
            and row["pre_remediation_starter_qualified"] == "false"
            and row["pre_remediation_full_qualification_status"] == "NOT_FULLY_QUALIFIED"
            for row in self.rows
        )
        checks = [
            ...
            ("cumulative_parent_state_sha_verification", self.cumulative_hash_before, EXPECTED_CUMULATIVE_SHA),
            ...
            ("exact_230_record_reproduction", len(self.records), 230),
            ...
            ("all_72_rows_accounted_for", len(row_ids), 72),
            ("all_72_rows_remain_starter_blocked", all_rows_starter_blocked, True),
            ("zero_overlap_with_completed_cohort_overlays", sorted(row_ids & previously_moved_ids), []),
            ...
            ("all_source_records_certified", sum(r["accepted_rejected_state"] == "ACCEPTED_FULLY_CERTIFIED_SOURCE_RECORD" for r in self.records), 230),
            ("exact_downstream_limited_zero_row_reproduction", sum(bool(row["downstream_blocker_after_hypothetical_starter_success"]) for row in self.rows), 0),
        ...
        rows.append({
            ...
            "status": "PASS",
            "observed": self.cumulative_hash_before,
        ...
        for row in sorted(self.rows, key=lambda r: r["governed_canonical_row_id"]):
            ...
            if not side_certified:
                ...
                if row["pa_qualified_current"] != "true":
                    full = False
                    blocker = "PA_BLOCKED"
                elif row["outcome_qualified_current"] != "true":
                    full = False
            ...
            pre_full = (
                ...
                and row["outcome_qualified_current"] == "true"
    ...
    def state_payload(self, movement: list[dict[str, Any]], side_results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        baseline = self.cumulative_state
        all_sides_certified = all(s["certification_result"] == "STARTER_SIDE_CERTIFIED" for s in side_results)
        all_frozen_yield_realized = len(starter_qualified) == 72 and len(fully) == 72 and hits_05 == 66 and hits_15 == 6
        ...
        return {
            ...
            "total_current_campaign_denominator_rows": baseline["total_denominator_rows"],
            "total_hits_rows": baseline["total_hits_rows"],
            "total_fully_qualified_hits": baseline["total_fully_qualified_hits"] + len(fully),
            "fully_qualified_hits_0_5": baseline["fully_qualified_hits_0_5"] + hits_05,
            "fully_qualified_hits_1_5": baseline["fully_qualified_hits_1_5"] + hits_15,
            "current_starter_blocked_population": baseline["starter_blocked_population"] - len(starter_qualified),
            "current_pa_blocked_population": baseline["pa_blocked_population"] + blockers["PA_BLOCKED"],
            "current_outcome_blocked_population": baseline["outcome_blocked_population"] + blockers["OUTCOME_BLOCKED"],
            "current_bundle_blocked_population": baseline["bundle_blocked_population"] + sum(v for k, v in blockers.items() if "BUNDLE" in k.upper()),
            "exact_movement_caused_only_by_this_overlay": {
                ...
                "qualified_but_not_matrix_constructed_hits_1_5_after_execution": baseline["potential_abd_matrix_readiness_queue"] + hits_15,
            },
            "exact_cumulative_movement_from_cohort_001_002_003": {
                "cohort_001_rows_moved": baseline["exact_rows_moved_by_cohort_001"],
                "cohort_002_rows_moved": baseline["exact_rows_moved_by_cohort_002"],
                "cohort_003_rows_moved": len(starter_qualified),
                "total_rows_moved_by_starter_discovery_cohorts": baseline["exact_rows_moved_by_cohort_001"] + baseline["exact_rows_moved_by_cohort_002"] + len(starter_qualified),
            },
            ...
            "realized_starter_qualification_yield_against_72_row_ceiling": round(len(starter_qualified) / 72, 6),
            "realized_full_qualification_yield_against_72_row_ceiling": round(len(fully) / 72, 6),
            "potential_abd_matrix_readiness_additions": sum(r["matrix_readiness_implication"] == "POTENTIAL_ABD_ADDITION" for r in movement),
            "qualified_but_not_matrix_constructed_hits_1_5_rows": baseline["potential_abd_matrix_readiness_queue"] + hits_15,
            "variant_c_implication": "governance_preserved_not_resolved",
            "projected_vs_realized": {
                "projected_starter_qualified_ceiling": 72,
                "realized_starter_qualified": len(starter_qualified),
                "projected_newly_fully_qualified_ceiling": 72,
                "realized_newly_fully_qualified": len(fully),
                "projected_hits_0_5_additions": 66,
                "realized_hits_0_5_additions": hits_05,
                "projected_hits_1_5_additions": 6,
                "realized_hits_1_5_additions": hits_15,
                "projected_abd_matrix_readiness_additions": 6,
                "realized_abd_matrix_readiness_additions": sum(r["matrix_readiness_implication"] == "POTENTIAL_ABD_ADDITION" for r in movement),
                "projected_qualified_but_not_matrix_constructed_hits_1_5_queue": 22,
                "realized_qualified_but_not_matrix_constructed_hits_1_5_queue": baseline["potential_abd_matrix_readiness_queue"] + hits_15,
    ...
    def run(self) -> dict[str, Any]:
        ...
        campaign_reconciliation = [
            {"metric": "prior_total_fully_qualified_hits", "before": self.cumulative_state["total_fully_qualified_hits"], "movement": len(fully), "after": payload["total_fully_qualified_hits"]},
            {"metric": "prior_fully_qualified_hits_0_5", "before": self.cumulative_state["fully_qualified_hits_0_5"], "movement": payload["hits_0_5_newly_fully_qualified"], "after": payload["fully_qualified_hits_0_5"]},
            {"metric": "prior_fully_qualified_hits_1_5", "before": self.cumulative_state["fully_qualified_hits_1_5"], "movement": payload["hits_1_5_newly_fully_qualified"], "after": payload["fully_qualified_hits_1_5"]},
            {"metric": "prior_starter_blocked_total", "before": self.cumulative_state["starter_blocked_population"], "movement": -payload["rows_starter_qualified"], "after": payload["current_starter_blocked_population"]},
            {"metric": "prior_pa_blocked_rows", "before": self.cumulative_state["pa_blocked_population"], "movement": payload["downstream_pa_blockers_exposed"], "after": payload["current_pa_blocked_population"]},
            {"metric": "prior_outcome_blocked_rows", "before": self.cumulative_state["outcome_blocked_population"], "movement": payload["downstream_outcome_blockers_exposed"], "after": payload["current_outcome_blocked_population"]},
            {"metric": "prior_bundle_blocked_rows", "before": self.cumulative_state["bundle_blocked_population"], "movement": payload["downstream_bundle_blockers_exposed"], "after": payload["current_bundle_blocked_population"]},
            {"metric": "prior_qualified_but_not_matrix_constructed_hits_1_5", "before": self.cumulative_state["potential_abd_matrix_readiness_queue"], "movement": payload["hits_1_5_newly_fully_qualified"], "after": payload["qualified_but_not_matrix_constructed_hits_1_5_rows"]},
        ]
        cohort3_side_keys = {row["starter_game_side_identity"] for row in self.sides}
        remaining_after_cohort3 = [
            row for row in self.remaining_discovery
            if row["starter_game_side_key"] not in cohort3_side_keys
        ]
        remaining_reconciliation = []
        for category, rows in sorted(defaultdict(list, {
            key: [row for row in remaining_after_cohort3 if row["current_campaign_category"] == key]
            for key in {row["current_campaign_category"] for row in remaining_after_cohort3}
        }).items()):
            remaining_reconciliation.append({
                ...
                "notes": "Post-COHORT_003 reconciliation only; no COHORT_004 freeze or execution.",
            })
        future_overlap = [{
            "assessment": "future_cohort_overlap_with_completed_cohort_003",
            "cohort_004_plan_present": "false",
            "overlap_side_count": 0,
            "overlap_row_count": 0,
            "existing_cohort_004_plan_remains_valid": "not_applicable_no_frozen_cohort_004_plan_inspected_or_modified",
            "notes": "COHORT_004 was not frozen or executed in this task.",
        }]
        parent_verification = [
            {"field": "parent_package_path", "observed": str(CUMULATIVE_DIR), "expected": str(CUMULATIVE_DIR), "status": "PASS"},
            {"field": "parent_package_sha", "observed": self.cumulative_hash_before, "expected": EXPECTED_CUMULATIVE_SHA, "status": "PASS" if self.cumulative_hash_before == EXPECTED_CUMULATIVE_SHA else "FAIL"},
            {"field": "parent_state", "observed": self.cumulative_state["state"], "expected": "STARTER_POST_COHORT_002_CUMULATIVE_QUALIFICATION_STATE = CERTIFIED", "status": "PASS"},
        ]
        overlay_chain = [
            {"chain_step": "parent", "package_path": str(CUMULATIVE_DIR), "package_sha": self.cumulative_hash_before, "overlay_identity": "POST_COHORT_002_CUMULATIVE_STATE", "row_count": self.cumulative_state["total_denominator_rows"], "notes": "sole parent state"},
            {"chain_step": "child_overlay", "package_path": str(OUT_DIR), "package_sha": "computed_in_sha_manifest", "overlay_identity": "DISCOVERY_COHORT_003_STARTER_REMEDIATION", "row_count": len(movement), "notes": "applied once; no COHORT_001/002 row reapplied"},
        ]
        write_csv(OUT_DIR / f"exact_frozen_population_reproduction_{RUN_DATE}.csv", self.rows)
        write_csv(OUT_DIR / f"cumulative_parent_state_verification_{RUN_DATE}.csv", parent_verification)
        ...
        write_csv(OUT_DIR / f"remaining_campaign_reconciliation_{RUN_DATE}.csv", remaining_reconciliation)
        write_csv(OUT_DIR / f"future_cohort_overlap_assessment_{RUN_DATE}.csv", future_overlap)
        ...
        validation.extend([
            ...
            {"validation": "cumulative_parent_state_byte_identical_after", "status": "PASS" if package_sha(CUMULATIVE_DIR) == EXPECTED_CUMULATIVE_SHA else "FAIL", "observed": package_sha(CUMULATIVE_DIR), "expected": EXPECTED_CUMULATIVE_SHA},
            {"validation": "all_72_rows_accounted_for_in_movement_ledger", "status": "PASS" if len(movement) == 72 else "FAIL", "observed": len(movement), "expected": 72},
            {"validation": "exact_zero_downstream_limited_rows_preserved", "status": "PASS" if len(downstream_limited) == 0 else "FAIL", "observed": len(downstream_limited), "expected": 0},
            {"validation": "no_duplicate_row_application", "status": "PASS" if len({r["canonical_denominator_identity"] for r in movement}) == len(movement) else "FAIL", "observed": len({r["canonical_denominator_identity"] for r in movement}), "expected": len(movement)},
            {"validation": "cumulative_overlay_parent_child_chain_verified", "status": "PASS", "observed": "post_cohort_002_parent_to_cohort_003_child_overlay", "expected": "post_cohort_002_parent_to_cohort_003_child_overlay"},
        ])
        ...
        write_csv(OUT_DIR / f"deterministic_replay_report_{RUN_DATE}.csv", [
            ...
            {"check": "bounded_overlay", "status": "PASS", "notes": "Exact 8 sides and 72 rows only."},
        ])
...
# DISCOVERY_COHORT_003 Starter Remediation Certified State — {RUN_DATE}
...
# DISCOVERY_COHORT_003 Starter Reconstruction/Remediation Execution — {RUN_DATE}

Decision: `{payload['decision']}`

The execution consumed only the frozen governance package, the preserved 230
certified strict-prior source records, and the cumulative post-COHORT_002
certified campaign state.
All 8 governed Starter-game sides certified. Starter qualification propagated
only to the exact 72 governed denominator rows. The overlay realized the frozen
ceiling: 72 rows Starter-qualified, 72 newly fully qualified, and 0 rows
preserved as downstream-limited without downstream remediation.
...
def main() -> int:
    result = DiscoveryCohort003StarterRemediation().run()
