...
MODEL_OUTPUT_PATH = "models/Strikeouts (Pitching)_model.pkl"
...
def fetch_data():
    response = (
        supabase
        .table("model_training_props")
        .select("*")
        .eq("prop_type", "Strikeouts (Pitching)")
...
# --- Main ---
def main():
    print("📦 Fetching data for 'Strikeouts (Pitching)'...")
    df = fetch_data()
    if df.empty:
