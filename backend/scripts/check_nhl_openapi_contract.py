...
def _nhl_paths(spec: Dict[str, Any]) -> Dict[str, Any]:
    return {p: v for p, v in (spec.get("paths") or {}).items() if _nhl_path(p)}
...
def diff_contract(snapshot: Dict[str, Any], current: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    s_paths = _nhl_paths(snapshot)
    c_paths = _nhl_paths(current)
    ...
    if missing_paths:
        errors.append(f"Missing NHL paths: {missing_paths}")
    if new_paths:
        errors.append(f"New NHL paths not in snapshot: {new_paths}")
