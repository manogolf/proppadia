...
CSV_PATH = Path(__file__).with_name("train_batter_hrr.csv")
OUT_PATH = Path(__file__).with_name("pred_hrr_test.csv")
TEST_DAYS = 21
VAL_DAYS = 14
...
print(f"Test prevalenceHRR≥1.5 = {prev15:.3f}  |  Constant baseline Brier = {brier_const15:.4f}")
...
with open(models_dir / "hrr_calibrators_v1.json", "w") as f:
    json.dump(calibrators, f, indent=2)
print(f"Saved calibrators to {models_dir/'hrr_calibrators_v1.json'}")


# Write predictions CSV
out.to_csv(OUT_PATH, index=False)
print(f"\nWrote {OUT_PATH} with calibrated probabilities for 0.5 / 1.5 / 2.5 (HRR)")
